...
const [balloonsPoppedCount, setBalloonsPopppedCount] = useState<number>(0)
...
const popBalloon = (index:number) => {
    if (index === balloonsPoppedCount) {
        setBalloonsPopppedCount(prev => prev + 1)
    }
}
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-4">
        ...
        <Card className="mx-auto overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl border-2 border-black">
            <CardHeader className="text-center">
                ...
                <CardDescription className="text-2xl font-semibold text-gray-600">Friend</CardDescription>
                <p className="text-lg text-gray-500">September 26th</p>
            </CardHeader>
                        ...
                        {[...Array(totalCandles)].map((_, index) => (
                            <AnimatePresence key={index}>
                                {(celebrating && index <= candlesLit) || (!celebrating && index < candlesLit) ? (
                                    ...
                                    animate={{scale:1}}
                                    exit={{scale:0}}
                                    transition={{duration:0.5, delay:celebrating ? index * 0.0 : 0}}
                                    >
                                    <FaBirthdayCake
